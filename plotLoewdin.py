...
def get_dataframe(block, nmax=None, only_occ=False):
    ...
    for line in block_iterator:
        ...
        # parse data lines
        n_entries = len(row_idx)
        contr = [ i for i in l[- n_entries:] ]
        a_no = l[0]
        ele = l[1]
        ao = l[2]
        ao_l = ao[0]
        ...
        elif only_occ and 0.0 in arr2[:, 2]:
            first_unocc = int(np.argwhere( arr2[:, 2] == 0.0 )[0])
            arr2 = arr2[:first_unocc]
            arr1 = arr1[:first_unocc]
            index = index[:first_unocc]
            break
    ...
    df2.loc[:, 'mo'] = df2.loc[:, 'mo'].astype(int)
...
def plot(dfarr, path=None, annot=True, s=1.0):
    ...
    minx, miny = 2 * s, 5 * s
    x = minx if x / minx < 1 else x
    y = miny if n * y / miny < 1 else n * y
    
    fig, axarr = plt.subplots(nrows=n,) # figsize=(x, y))
    try:
        len(axarr)
    except TypeError:
        axarr = [ axarr ]
